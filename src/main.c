...
void app_main() {
    ...
    ESP_LOGI(TAG_MAIN, "Starting main");

    blue_led.pin = GPIO_NUM_2;
    led_init(&blue_led);

    ESP_LOGI(TAG_MAIN, "Led init");

    ble_gatt_profile_req_t * profs;
    uint16_t num_profs;
    ret = ble_profile_req(&profs, &num_profs);
    ESP_ERROR_CHECK(ret);
    ESP_LOGI(TAG_MAIN, "Profs init");

    ret = ble_init(profs, num_profs);
    ESP_ERROR_CHECK(ret);
    ...
    while (1) 
    {
        ...
        if (gattd_dispense_amounts[0].hour > 127)
        {
